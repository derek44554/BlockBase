...
def pin_cid(cid):
    ...
    :return:
    """
    response = requests.post(f"{ipfs_api}/pin/add", params={"arg": cid}, timeout=60)
    if response.status_code != 200:
        raise
...
def unpin_cid(cid):
    ...
    :return:
    """
    response = requests.post(f"{ipfs_api}/pin/rm", params={"arg": cid}, timeout=60)
    if response.status_code != 200:
        raise
...
def get_file_chunk_from_ipfs(cid, chunk_size=1 * 1024 * 1024):
    ...
    response = requests.post(ipfs_url, params={"arg": cid}, timeout=60)
